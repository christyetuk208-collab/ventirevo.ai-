...
export const AdminDashboardView: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'flags' | 'users' | 'errors' | 'limits'>('overview');
    ...
    try {
      ...
      if (res.ok) {
        setFeatureFlags((prev) =>
          prev.map((f) => (f.id === flagId ? { ...f, enabled: !currentStatus } : f))
        );
        setActionMessage(`Feature flag successfully ${!currentStatus ? 'enabled' : 'disabled'}.`);
  ...
  const clearSystemCache = async () => {
    try {
      const res = await fetch('/api/admin/maintenance/cache-clear', {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        setActionMessage('System cache and in-memory indexes successfully recycled.');
  ...
  const updateUserRole = async (targetUserId: string, newRole: string) => {
    try {
      ...
      if (res.ok) {
        ...
        setActionMessage(`User permissions updated to '${newRole}'.`);
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-400 mt-0.5">
              Production telemetry, subscription distribution, feature toggles, and live usage governance.
            </p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-1.5 p-1 rounded-xl bg-neutral-900/80 border border-neutral-800 overflow-x-auto">
        ...
        >
          ...
          <span>Errors & Resilience ({metrics?.recent_errors?.length || 0})</span>
        </button>
      </div>
      ...
      {isLoading ? (
        <div className="p-12 text-center text-neutral-400 space-y-2">
          ...
          <p className="text-xs">Aggregating real-time database state...</p>
        </div>
      ) : metrics ? (
        <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="p-4 rounded-2xl bg-neutral-900/60 border border-neutral-800">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-neutral-400">Total Registered Users</span>
                ...
                <div className="p-4 rounded-2xl bg-neutral-900/60 border border-neutral-800">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-neutral-400">Active Subscriptions</span>
                  ...
                  <div className="mt-2 flex items-baseline gap-2">
                    ...
                    <span className="text-[11px] text-blue-400 font-semibold">Paid / Pro</span>
                  </div>
                  <div className="mt-2 flex items-center gap-2 text-[11px] text-neutral-400">
                    <span>Free: {metrics.plan_distribution.free}</span>
                    <span>•</span>
                    <span>Pro: {metrics.plan_distribution.pro}</span>
                    <span>•</span>
                    <span>Max: {metrics.plan_distribution.max}</span>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-2xl bg-neutral-900/60 border border-neutral-800">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-neutral-400">Verified Revenue</span>
                ...
                <div className="p-4 rounded-2xl bg-neutral-900/60 border border-neutral-800">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-neutral-400">AI Compute Cost</span>
                  ...
                  <div className="mt-2 text-[11px] text-neutral-500">
                    Tokens: {metrics.ai_usage.total_tokens.toLocaleString()}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="p-5 rounded-2xl bg-neutral-900/60 border border-neutral-800 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] text-neutral-500">Live counts</span>
                  </div>

                  <div className="space-y-2.5">
                    {Object.entries(metrics.feature_usage).map(([key, count]) => {
                      const maxVal = Math.max(...Object.values(metrics.feature_usage), 1);
                      const pct = Math.round((Number(count) / maxVal) * 100);
                      const formatName = key.replace(/_/g, ' ').toUpperCase();

                      return (
                        <div key={key} className="space-y-1">
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-neutral-300 capitalize">{formatName}</span>
                            <span className="font-semibold text-emerald-400">{count} events</span>
                          </div>
                ...
                <div className="p-5 rounded-2xl bg-neutral-900/60 border border-neutral-800 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 rounded-xl bg-neutral-950/60 border border-neutral-800/80">
                      <span className="text-[10px] font-medium text-neutral-500 uppercase">Process Uptime</span>
                      <p className="text-sm font-bold text-neutral-200 mt-1">
                        {Math.floor(metrics.system_health.uptime_seconds / 60)} mins
                      </p>
                    </div>

                    <div className="p-3 rounded-xl bg-neutral-950/60 border border-neutral-800/80">
                      <span className="text-[10px] font-medium text-neutral-500 uppercase">Memory Heap</span>
                      <p className="text-sm font-bold text-neutral-200 mt-1">
                    ...
                    <div className="p-3 rounded-xl bg-neutral-950/60 border border-neutral-800/80">
                      <span className="text-[10px] font-medium text-neutral-500 uppercase">API Latency</span>
                    ...
                    <div className="p-3 rounded-xl bg-neutral-950/60 border border-neutral-800/80">
                      <span className="text-[10px] font-medium text-neutral-500 uppercase">Ventirevo Neural Engine</span>
                      <p className="text-sm font-bold text-neutral-200 mt-1">
                        Active & Guarded
                      </p>
                    </div>
                  ...
                  <div className="p-3 rounded-xl bg-emerald-950/30 border border-emerald-800/40 text-[11px] text-neutral-300">
                    <span className="font-semibold text-emerald-400">Zero-Hallucination Policy:</span> All metric totals are computed directly from deterministic session records and validated Stripe invoices without synthetic padding.
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'flags' && (
            <div className="space-y-4">
              <div className="p-4 rounded-2xl bg-neutral-900/60 border border-neutral-800">
                <h3 className="text-sm font-bold text-neutral-100">Live Feature Governance</h3>
                <p className="text-xs text-neutral-400 mt-0.5">
                  Toggle runtime capabilities without redeploying backend containers.
                </p>
              </div>
                ...
                {featureFlags.map((flag) => (
                  ...
                  >
                    <div className="space-y-1 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-xs text-neutral-200">{flag.name}</span>
          ...
          {activeTab === 'users' && (
            <div className="space-y-4">
              <div className="p-4 rounded-2xl bg-neutral-900/60 border border-neutral-800 flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-neutral-100">User Access & Role Directory</h3>
                  <p className="text-xs text-neutral-400 mt-0.5">
                    Inspect founder profiles, subscription tiers, and assign admin roles.
                  </p>
                </div>
                  ...
                  <tbody className="divide-y divide-neutral-800/60 text-neutral-300">
                    {usersList.map((u) => (
                      <tr key={u.id} className="hover:bg-neutral-800/30 transition">
                        <td className="p-3.5">
                          <div className="font-semibold text-neutral-100">{u.name || 'Founder'}</div>
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-2xl bg-neutral-900/60 border border-neutral-800">
                <h3 className="text-sm font-bold text-neutral-100 flex items-center gap-2">
                  ...
                  <span>Telemetry & Handled Exception Logs</span>
                </h3>
                <p className="text-xs text-neutral-400 mt-0.5">
                  Transient upstream load spikes, handled fallbacks, and recovery transactions.
                </p>
              </div>
              ...
              {metrics.recent_errors.length === 0 ? (
                <div className="p-8 text-center rounded-2xl bg-neutral-900/30 border border-neutral-800 text-neutral-400 text-xs">
                  ...
                  <span>Zero unhandled production exceptions detected in current cycle.</span>
                </div>
              ) : (
                <div className="space-y-2.5">
                      ...
                      <div className="flex-1 space-y-1">
                        <div className="flex items-center justify-between">
                          <span className="font-mono text-xs font-bold text-amber-300">
                            {err.error_type} ({err.status_code || 500})
                          </span>
